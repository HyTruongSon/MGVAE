...
# Multiresolution (cluster) Graph Variational Autoencoder
class MGVAE_cluster(nn.Module):
    def __init__(self, adj, clusters, input_dim, hidden_dim, z_dim, device = 'cuda'):
        super(MGVAE_cluster, self).__init__()
        self.adj = adj
...
class GraphEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, z_dim, device = 'cuda', **kwargs):
        ...
        self.node_fc1 = nn.Linear(self.input_dim, 128).to(device = device)
        self.node_fc2 = nn.Linear(128, self.hidden_dim).to(device = device)

        self.base_net = GraphConvSparse(self.hidden_dim, self.hidden_dim)
        self.mean_net = GraphConvSparse(self.hidden_dim, self.z_dim)
        self.logstd_net = GraphConvSparse(self.hidden_dim, self.z_dim)

    def forward(self, adj, X):
        hidden = torch.tanh(self.node_fc1(X))
        hidden = torch.tanh(self.node_fc2(hidden))

        hidden = self.base_net(adj, hidden)
...
class GraphCluster(nn.Module):
    def __init__(self, input_dim, hidden_dim, z_dim, device = 'cuda', **kwargs):
        ...
        self.base_net = GraphConvSparse(self.hidden_dim, self.hidden_dim)
        self.assign_net = GraphConvSparse(self.hidden_dim, self.z_dim)
...
class GraphConvSparse(nn.Module):
    def __init__(self, input_dim, output_dim, activation = torch.tanh, **kwargs):
...
model = MGVAE_cluster(input_adj, clusters, input_dim, hidden_dim, z_dim, device = device).to(device=device)
...
def multiresolution_loss(outputs, val_edges, val_edges_false, test_edges, test_edges_false,  norm, adj_label, weight_tensor, device, args):
    for i in range(len(outputs)):
        ...
        if i == 0:
            ...
            removed_edges = np.concatenate((val_edges, val_edges_false, test_edges, test_edges_false), axis = 0)
            N = adj_label.size(0)
            idx1 = np.zeros(removed_edges.shape[0])
            idx1 = removed_edges[:, 0] * N + removed_edges[:, 1]
            idx2 = np.zeros(removed_edges.shape[0])
            idx2 = removed_edges[:, 1] * N + removed_edges[:, 0]
            removed_idx = np.concatenate([idx1, idx2], axis = 0)
            mask = np.full(N * N, True)
            mask[removed_idx] = False
            mask = torch.BoolTensor(mask).to(device = device)

            # Prediction on the train set
            rec = torch.flatten(adj_rec)
            label = torch.flatten(adj_label.to_dense())
            weight = torch.flatten(weight_tensor)
            ...
            # Train accuracy
            train_acc = get_acc(rec, label.to(device = device))

            # Binary cross entropy loss
            loss = norm * F.binary_cross_entropy(rec.view(-1), label.view(-1).to(device = device), weight = weight)
        else:
...
for epoch in range(args.num_epoch):
    ...
    optimizer.zero_grad()
    loss, adj_rec, train_acc = multiresolution_loss(outputs, val_edges, val_edges_false, test_edges, test_edges_false, norm, adj_label, weight_tensor, device, args)
    loss.backward()
    optimizer.step()

    # train_acc = get_acc(adj_rec, adj_label.to(device = device))

    val_roc, val_ap = get_scores(val_edges, val_edges_false, adj_rec.cpu())
    ...
    if val_roc > best_val_roc:
        ...
        test_roc, test_ap = get_scores(test_edges, test_edges_false, adj_rec.cpu())
