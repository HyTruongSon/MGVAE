...
# Multiresolution (Second order) Graph Variational Autoencoder
class MGVAE_2nd(nn.Module):
    def __init__(self, adj, tree, targets, input_dim, hidden_dim, z_dim, decoder_channels, outer_type, device = 'cuda'):
        ...
        for l in range(self.n_levels + 1):
            ...
            self.local_decoder.append(SnDecoder(self.z_dim, self.decoder_channels, 1, outer_type = self.outer_type).to(device = device))
        ...
        while l >= 0:
            if l == self.n_levels:
                ...
                for i in self.tree.levels[l]:
                    ...
                    # Local Sn decoder
                    num_vertices = local_adj.size(0)
                    predict = self.local_decoder[l](torch.reshape(latent, (1, num_vertices, self.z_dim)))
                    predict = torch.reshape(torch.sigmoid(predict), (num_vertices, num_vertices))
            ...
            num_vertices = global_adj.size(0)
            predict = self.global_decoder[l](torch.reshape(next_latent, (1, num_vertices, self.z_dim)))
            predict = torch.reshape(torch.sigmoid(predict), (num_vertices, num_vertices))
...
model = MGVAE_2nd(input_adj, tree, targets, input_dim, hidden_dim, z_dim, decoder_channels, outer_type, device = device).to(device=device)
