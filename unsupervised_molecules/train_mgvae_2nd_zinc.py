...
def _parse_args():
    ...
    parser.add_argument('--dataset', '-dataset', type = str, default = 'ZINC_smiles', help = 'QM9_smiles/ZINC_smiles')
    ...
    parser.add_argument('--pos_weight', type = float, default = 100., help = 'Positive weight')
...
# Dataset
dataset = zinc_dataset(directory = args.dataset, on_the_fly = True)
...
model = MGVAE_2nd(clusters = clusters, num_layers = args.n_layers, node_dim = node_dim, edge_dim = None, hidden_dim = args.hidden_dim, z_dim = args.z_dim, decoder_channels = decoder_channels, outer_type = outer_type, device = device).to(device=device)
...
for epoch in range(args.num_epoch):
    ...
    for _, data in enumerate(dataloader):
        ...
        if nBatch < len(target):
            assert torch.sum(torch.abs(target[nBatch] - data['logp'])) < 1e-6
