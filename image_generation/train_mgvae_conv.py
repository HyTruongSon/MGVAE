...
from mnist_rotation_dataset import mnist_rotation_dataset

def _parse_args():
    parser = argparse.ArgumentParser(description = 'Image generation')
    parser.add_argument('--dir', '-dir', type = str, default = '.', help = 'Directory')
    parser.add_argument('--limit_data', '-limit_data', type = int, default = -1, help = 'Limit data')
...
# Deconvolution to 32x32
class Deconvolution_32(nn.Module):
    def __init__(self, z_dim, output_dim, hidden_dim = 256):
        super(Deconvolution_32, self).__init__()
        self.z_dim = z_dim
    ...
    def forward(self, input):
        ...
        input = torch.reshape(input, (batch_size, input.size(1), 1, 1))
        output = self.main(input)
        return output.transpose(1, 2).transpose(2, 3)

# Deconvolution to 16x16
class Deconvolution_16(nn.Module):
    def __init__(self, z_dim, output_dim, hidden_dim = 256):
        super(Deconvolution_16, self).__init__()
        self.z_dim = z_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
        self.main = nn.Sequential(
...
# Deconvolution to 8x8
class Deconvolution_8(nn.Module):
    def __init__(self, z_dim, output_dim, hidden_dim = 256):
        super(Deconvolution_8, self).__init__()
        self.z_dim = z_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
        self.main = nn.Sequential(
...
class MLP_Decoder(nn.Module):
    def __init__(self, input_dim, output_dim, height = 32, width = 32, hidden_dim = 512):
        ...
        self.layers.append(nn.Linear(self.hidden_dim, self.hidden_dim))
        self.layers.append(nn.Tanh())
        self.layers.append(nn.Linear(self.hidden_dim, self.height * self.width * self.output_dim))
        self.layers.append(nn.Sigmoid())
        self.fc = nn.Sequential(*self.layers)
...
class MGVAE(nn.Module):
    def __init__(self, image_height, image_width, cluster_height, cluster_width, n_layers, n_levels, input_dim, hidden_dim, z_dim, device = 'cuda'):
        ...
        # Base encoder
        self.base_encoder = GraphEncoder(self.base_adj, self.n_layers, self.input_dim, None, self.hidden_dim, self.z_dim, device = device).to(device = device)
        ...
        # Convolution decoder
        self.base_decoder = Deconvolution_32(self.z_dim, self.input_dim).to(device = device)
        ...
        for l in range(self.n_levels):
            ...
            self.global_encoder.append(GraphEncoder(self.global_adj[l], self.n_layers, self.z_dim, None, self.hidden_dim, self.z_dim, device = device).to(device = device))
    ...
    def forward(self, image):
        ...
        # Base encoder
        base_features, base_latent, base_mean, base_logvar = self.base_encoder(node_features)
        ...
        while l >= 0:
            ...
            # Global encoder
            next_features, next_latent, next_mean, next_logvar = self.global_encoder[l](node_features)
...
class GraphEncoder(nn.Module):
    def __init__(self, adj, num_layers, node_dim, edge_dim, hidden_dim, z_dim, use_concat_layer = True, device = 'cuda', vae = True, **kwargs):
        super(GraphEncoder, self).__init__(**kwargs)
        self.adj = adj
        self.num_vertices = self.adj.size(0)
        ...
        if self.use_concat_layer == False:
            ...
            self.mean_fc2 = nn.Linear(128, self.z_dim).to(device = device)
            self.mean_fc3 = nn.Linear(self.z_dim, 16).to(device = device)
            self.mean_fc4 = nn.Linear(16 * self.num_vertices, self.z_dim).to(device = device)

            if self.vae is True:
                self.logvar_fc1 = nn.Linear((self.num_layers + 1) * self.hidden_dim, 128).to(device = device)
                self.logvar_fc2 = nn.Linear(128, self.z_dim).to(device = device)
                self.logvar_fc3 = nn.Linear(self.z_dim, 16).to(device = device)
                self.logvar_fc4 = nn.Linear(16 * self.num_vertices, self.z_dim).to(device = device)
    ...
    def forward(self, node_feat, edge_feat = None):
        ...
        for layer in range(len(self.base_net)):
            if layer == 0:
                hidden = self.base_net[layer](self.adj, node_hidden)
            else:
                hidden = self.base_net[layer](self.adj, hidden)
        ...
        mean = torch.tanh(self.mean_fc2(mean))
        features = mean
        
        # Option 1
        # mean = torch.tanh(self.mean_fc3(mean.transpose(1, 2)))
        # mean = self.mean_fc4(mean).squeeze(dim = 2)

        # Option 2
        # mean = torch.mean(mean, dim = 1)

        # Option 3
        mean = torch.tanh(self.mean_fc3(mean))
        mean = torch.reshape(mean, (mean.size(0), mean.size(1) * mean.size(2)))
        mean = self.mean_fc4(mean)

        logvar = torch.tanh(self.logvar_fc1(hidden))
        logvar = torch.tanh(self.logvar_fc2(logvar))
        
        # Option 1
        # logvar = torch.tanh(self.logvar_fc3(logvar.transpose(1, 2)))
        # logvar = self.logvar_fc4(logvar).squeeze(dim = 2)

        # Option 2
        # logvar = torch.mean(logvar, dim = 1)

        # Option 3
        logvar = torch.tanh(self.logvar_fc3(logvar))
        logvar = torch.reshape(logvar, (logvar.size(0), logvar.size(1) * logvar.size(2)))
        logvar = self.mean_fc4(logvar)
...
if args.dataset == 'cifar10':
    ...
    if args.dataset == 'mnist':
        ...
        if args.dataset == 'mnist_rotation':
            trainset = mnist_rotation_dataset(directory = './mnist_rotation', split = 'train', limit_data = args.limit_data)
            testset = mnist_rotation_dataset(directory = './mnist_rotation', split = 'test', limit_data = args.limit_data)
...
for epoch in range(args.num_epoch):
    ...
    print("Model is saved to " + args.dir + "/" + args.name + ".model")
        ...
        for _, data in enumerate(test_dataloader):
            ...
            for b in range(image.size(0)):
                ...
                for l in range(len(all_rec)):
                    ...
                    plt.savefig(args.dir + "/visualization/idx_" + str(idx) + "_target_resolution_" + str(args.n_levels - l) + ".png")
                    ...
                    plt.savefig(args.dir + "/visualization/idx_" + str(idx) + "_rec_resolution_" + str(args.n_levels - l) + ".png")
                idx += 1
    ...
    with torch.no_grad():
        ...
        for _, data in enumerate(test_dataloader):
            ...
            while l >= 0:
                ...
                latent = torch.randn(batch_size, args.z_dim).to(device = device)
        ...
        name = args.dir + "/" + args.name

        all_base_predict = np.concatenate(all_base_predict, axis = 0)
        np.save(name + ".resolution_level_" + str(args.n_levels), all_base_predict)
