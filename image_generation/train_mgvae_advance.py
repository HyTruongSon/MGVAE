...
class MGVAE(nn.Module):
    def __init__(self, image_height, image_width, cluster_height, cluster_width, n_layers, n_levels, input_dim, hidden_dim, z_dim, device = 'cuda'):
        ...
        for l in range(self.n_levels):
            ...
            # Graph decoder
            self.global_decoder.append(GraphEncoder(self.n_layers, self.z_dim, None, self.hidden_dim, 3, device = device, vae = False).to(device = device))
    ...
    def forward(self, image):
        ...
        # Base decoder -- Graph decoder
        base_predict = torch.sigmoid(self.base_decoder(self.base_adj, base_latent))
        ...
        while l >= 0:
            ...
            # size = self.assign[l].size(0) * self.assign[l].size(1)
                       
            node_features = torch.einsum('ki,bic->bkc', self.assign[l].transpose(0, 1), prev_latent)

            # Global encoder
            next_latent, next_mean, next_logvar = self.global_encoder[l](self.global_adj[l], node_features)

            # Global decoder -- Graph decoder
            global_predict = torch.sigmoid(self.global_decoder[l](self.global_adj[l], next_latent))
           
            outputs.append([next_latent, next_mean, next_logvar, global_predict, target])

            prev_latent = next_latent
            prev_mean = next_mean
            prev_logvar = next_logvar
...
for epoch in range(args.num_epoch):
    ...
    with torch.no_grad():
        ...
        for _, data in enumerate(test_dataloader):
            ...
            # Base decoder -- Graph decoder
            base_predict = torch.sigmoid(model.base_decoder(model.base_adj, base_latent))
            ...
            while l >= 0:
                ...
                predict = torch.sigmoid(model.global_decoder[l](model.global_adj[l], latent))
