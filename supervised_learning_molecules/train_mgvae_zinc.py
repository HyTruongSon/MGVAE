...
from zinc_dataset import zinc_dataset
...
# Dataset
train_dataset = zinc_dataset(directory = args.dataset, split = 'new_train.index', on_the_fly = True, max_num_atoms = 38)
val_dataset = zinc_dataset(directory = args.dataset, split = 'new_val.index', on_the_fly = True, max_num_atoms = 38)
test_dataset = zinc_dataset(directory = args.dataset, split = 'new_test.index', on_the_fly = True, max_num_atoms = 38)
...
for epoch in range(args.num_epoch):
    ...
    with torch.no_grad():
        ...
        print('Val MAE:', mae)
        LOG.write('Val MAE: ' + str(mae) + '\n')
        print("Val time =", "{:.5f}".format(time.time() - t))
        LOG.write("Val time = " + "{:.5f}".format(time.time() - t) + "\n")
    
    # If we do better on the validation set
    if mae < best_val_mae:
        best_val_mae = mae
        print('Current best val MAE updated:', best_val_mae)
        LOG.write('Current best val MAE updated: ' + str(best_val_mae) + '\n')
        ...
        # Testing: Just in case the training is terminated in the middle
        t = time.time()
        model.eval()
        with torch.no_grad():
            sum_error = 0.0
...
LOG.write('Best val MAE: ' + str(best_val_mae) + '\n')
