...
from molhiv_dataset import molhiv_dataset

def _parse_args():
    parser = argparse.ArgumentParser(description = 'Supervised learning')
    parser.add_argument('--dir', '-dir', type = str, default = '.', help = 'Directory')
    parser.add_argument('--name', '-name', type = str, default = 'NAME', help = 'Name')
    parser.add_argument('--dataset', '-dataset', type = str, default = 'ogbg-molhiv', help = 'For example: ogbg-molhiv')
...
class MGVAE(nn.Module):
    ...
    def forward(self, adj, node_feat, edge_feat = None):
        ...
        predict = torch.sigmoid(self.fc2(hidden))
...
# Dataset
total_dataset = GraphPropPredDataset(name = args.dataset)

train_dataset = molhiv_dataset(dataset = total_dataset, split = 'train')
valid_dataset = molhiv_dataset(dataset = total_dataset, split = 'valid')
test_dataset = molhiv_dataset(dataset = total_dataset, split = 'test')
...
for epoch in range(args.num_epoch):
    ...
    for batch_idx, data in enumerate(train_dataloader):
        ...
        label = data['label'].float().to(device = device)

        predict, latent, outputs = model(adj, node_feat, edge_feat = None)
        optimizer.zero_grad()
        
        # Cross-entropy loss with positive weight
        loss = F.binary_cross_entropy(predict.view(-1), label.view(-1), reduction = 'mean', weight = pos_weight)

        # Cross-entropy loss without positive weight
        # loss = F.binary_cross_entropy(predict.view(-1), label.view(-1), reduction = 'mean')
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(valid_dataloader):
            ...
            node_feat = data['node_feat'].float().to(device = device)
            edge_feat = data['edge_feat'].float().to(device = device)
            label = data['label'].float().detach().cpu().numpy()
        ...
        N = all_label.shape[0]
        all_label = np.reshape(all_label, (N, 1))
        all_predict = np.reshape(all_predict, (N, 1))

        # Accuracy
        acc = np.mean(all_label == all_predict)

        # ROC/AUC
        input_dict = {"y_true": all_label, "y_pred": all_predict}
        result_dict = evaluator.eval(input_dict)
        roc = result_dict['rocauc']

        print('Valid accuracy:', acc)
        LOG.write('Valid accuracy: ' + str(acc) + '\n')
        print('Valid ROC/AUC:', roc)
        LOG.write('Valid ROC/AUC: ' + str(roc) + '\n')
    ...
    if roc > best_roc:
        best_roc = roc
        print('Current best ROC/AUC updated:', best_roc)
        LOG.write('Current best ROC/AUC updated: ' + str(best_roc) + '\n')
    ...
    with torch.no_grad():
        ...
        LOG.write('Test ROC/AUC: ' + str(roc) + '\n')
...
print('Best valid ROC/AUC:', best_roc)
LOG.write('Best valid ROC/AUC: ' + str(best_roc) + '\n')
print('Test ROC/AUC:', test_roc)
LOG.write('Test ROC/AUC: ' + str(test_roc) + '\n')
LOG.close()
